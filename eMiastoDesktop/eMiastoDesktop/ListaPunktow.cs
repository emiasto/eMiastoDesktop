	...
	public partial class ListaPunktow : Form {
		...
		private void btnDodaj_Click(object sender, EventArgs e) {
			...
			using (SimpleBrowserForm nowa = new SimpleBrowserForm()) {
				...
				nowa.ShowDialog();
				url = nowa.browser.Address;
			}
			if (url.IndexOf("/@") > 0 && url.IndexOf('z', url.IndexOf("/@")) > 0) {
					var wyciety = url.Substring(url.IndexOf("/@"), (url.IndexOf('z', url.IndexOf("/@")) - url.IndexOf("/@"))).Substring(2);
